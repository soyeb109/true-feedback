...
export async function POST(request: Request) {
  ...
  try {
    ...
    const exixtingUserVerifyByUsername = await UserModel.findOne({
      username,
      isVerified: true,
    });

    if (exixtingUserVerifyByUsername) {
    ...
    if (exixtingUserByEmail) {
      if (exixtingUserByEmail.isVerified) {
        ...
        exixtingUserByEmail.verifyCodeExpiry = new Date(
          Date.now() + 30 * 60 * 1000
        );
    ...
    return Response.json(
      { success: true, message: "Sign up successful, verification email sent" },
      { status: 201 }
    );
  } catch (error) {
